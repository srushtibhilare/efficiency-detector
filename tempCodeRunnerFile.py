...
# --- Function to Save Face Snapshot ---
...
elapsed_time = 0
timer_running = False
manual_control = False  # Track if we're in manual control mode
data_saved = False
face_detected = False
last_face_time = 0
...
face_timeout = 1.0  # Time before stopping when face disappears
...
while True:
    ...
    # Face detection logic (only if not in manual control)
    if not manual_control:
        ...
        if current_face_detected:
            ...
            # Only stop if face was previously detected and timeout has passed
            if timer_running and face_detected and (time.time() - last_face_time) > face_timeout:
                elapsed_time = time.time() - start_time
                timer_running = False
                # Save data when face disappears
                if not data_saved and elapsed_time > 0:
                    success, frame_to_save = cap.read()
                    if success:
                        frame_to_save = cv2.flip(frame_to_save, 1)
                        image_path = save_face_image(frame_to_save)
                        ...
                        conn.commit()
                        print(f"Data saved: {elapsed_time:.2f} seconds")
                    data_saved = True
                face_detected = False
    else:
        # If in manual control, reset face detected flag so auto-stop doesn't trigger immediately
        face_detected = False

    # Hand gesture control (overrides face detection)
    if hand_result.multi_hand_landmarks:
        ...
        # 2 fingers (index + middle) - STOP timer and enter manual control
                ...
                if not data_saved and elapsed_time > 0:
                    ...
                    if success:
                        ...
                        cursor.execute("INSERT INTO timer_logs (timestamp, elapsed_time, face_image_path) VALUES (?, ?, ?)",
                                       (timestamp, elapsed_time, image_path))
                        conn.commit()
                        print(f"Data saved: {elapsed_time:.2f} seconds (manual stop)")
                    data_saved = True

        # 3 fingers (index + middle + ring) - START timer and enter manual control
        ...
        # No specific hand gesture - exit manual control (face detection takes over)
    ...
    # Remove or comment out these lines to prevent drawing the face mesh
